...
public class PillarTrigger_Below : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D col)
    {
        ...
        {
            for (int i = 0; i < triggerHandler.SR_ObjBelow_List.Count; i++)
            {
                StartCoroutine("FadeOut", triggerHandler.SR_ObjBelow_List[i]);
            }
        }
    }
    private void OnTriggerExit2D(Collider2D col)
    {
        if (col.CompareTag("PlayerTrigger"))
        {
            for (int i = 0; i < triggerHandler.SR_ObjBelow_List.Count; i++)
            {
                StartCoroutine("FadeIn", triggerHandler.SR_ObjBelow_List[i]);
            }
        }
    }
